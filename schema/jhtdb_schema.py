...
class VariableTimeMethod(BaseModel):
    '''
    Which temporal interpolation methods can be applied to which variable.
    All represented by their codes pointing to the variables/operators/interpolation_methods in the database definition.
    '''
    variable: str  # code in a variable definition
    methods: List[str]  # list of codes in method definitions
...
class Dataset(BaseModel):
    ...
    variable_time_methods: List[VariableTimeMethod] 
...
if __name__ == "__main__":
    sch = TurbulenceDB.model_json_schema()  # (1)!
    with open("jhtdb-schema.json","w") as f:
        f.write(json.dumps(sch, indent=2))  # (2)!

    sch = JHTDBServerSide.model_json_schema()  # (1)!
    with open("jhtdb-server-schema.json","w") as f:
        f.write(json.dumps(sch, indent=2))  # (2)!

    sch = JHTDBCutout.model_json_schema()  # (1)!
    with open("jhtdb-cutout-schema.json","w") as f:
        f.write(json.dumps(sch, indent=2))  # (2)!

    sch = JHTDBPointQuery.model_json_schema()  # (1)!
    with open("jhtdb-points-schema.json","w") as f:
        f.write(json.dumps(sch, indent=2))  # (2)!